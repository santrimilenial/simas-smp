<x-layout>
    ...
    <x-slot name="header">Laporan Jurnal Mengajar</x-slot>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        <form method="GET" action="{{ route('admin.reports.index') }}" class="mb-6">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Filter Guru</label>
                    <select 
                        name="guru_id"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                        <option value="">Semua Guru</option>
                        @foreach(\App\Models\User::guru()->orderBy('name')->get() as $guru)
        ...
        <div class="flex gap-3 mb-6">
            <a href="{{ route('admin.reports.pdf', request()->query()) }}" class="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg font-semibold">
                <i class="fas fa-file-pdf mr-2"></i>Export PDF
            </a>
            <a href="{{ route('admin.reports.excel', request()->query()) }}" class="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg font-semibold">
                        ...
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="py-3 px-4">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ $log->user->niy }}</p>
                                </div>
                            </td>
        ...
        <div class="mt-6 bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
            <p class="text-sm text-gray-700">
                <strong>Total:</strong> {{ $logs->total() }} jurnal ditemukan
            </p>
        </div>
        @endif
    </div>
